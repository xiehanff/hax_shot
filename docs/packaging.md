...
```text
pubspec 1.6.0+1  →  tag v1.6.0
  macOS   HaxShot-1.6.0-arm64.dmg（没签名/公证时必须叫 HaxShot-1.6.0-arm64-unsigned.dmg）
  Windows HaxShot-1.6.0-windows-x64-setup.exe
          HaxShot-1.6.0-windows-x64.zip
  DEB     hax-shot_1.6.0+1_amd64.deb
  RPM     hax-shot-1.6.0-1.x86_64.rpm
```
...
|---|---|---|
| `1.6.0+1` | `v1.6.0` | `HaxShot-1.6.0-arm64.dmg`、`HaxShot-1.6.0-windows-x64-setup.exe`、`HaxShot-1.6.0-windows-x64.zip`、`hax-shot_1.6.0+1_amd64.deb`、`hax-shot-1.6.0-1.x86_64.rpm` |

其中：

- `1.6.0` 是应用版本，也是 DMG 名字、Windows 两个产物名字和 RPM 的 Version；
- `+1` 是 DEB 的完整版本（`1.6.0+1`）和 Fedora RPM 的 Release；
- RPM 在 Fedora 本机构建时会带 `.fc44`，在 CI（Ubuntu 的 rpmbuild）里没有这个发行版后缀；
- `v1.6.0` 是 GitHub Release 的 tag 和页面名称。
...
```bash
hdiutil attach HaxShot-1.6.0-arm64.dmg          # 能挂载且内含 HaxShot.app
...
```bash
sudo dnf install ./hax-shot-1.6.0-1.x86_64.rpm        # 或 sudo apt install ./hax-shot_1.6.0+1_amd64.deb
/usr/share/hax-shot/install-gnome-shortcut.sh
```
...
```powershell
pwsh scripts/verify_windows_installer.ps1 -Installer HaxShot-1.6.0-windows-x64-setup.exe
```
