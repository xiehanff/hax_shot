...
“修改快捷键”会临时显示一个设置窗口，而不是打开外部控制中心。设置窗口展示当前 GNOME 快捷键，右侧 `×` 清空快捷键，点击“录制新的快捷键”后聚焦键盘录制区域；按下带修饰键的组合键后立即写回 GNOME GSettings，按 `Esc` 取消录制。关闭设置窗口后，宿主仍回到纯托盘状态。
...
当前 `CaptureToolbar` 包含取消、保存、复制三个控件，并预留框选、箭头、文字、颜色板（红/紫/黄/绿/橙）和 AI（翻译/解释/深度理解）控件。图标统一使用 `hugeicons` 的 `strokeRounded` 风格；点击颜色色斑会切换对应颜色边框，保存和复制是当前可用的主要动作，其他标注/AI 控件暂不执行操作。拖拽中的临时矩形仍然绘制边框，但工具栏要等 `selectionCommitted` 在 `onPanEnd` 中变为 true 后才显示。开始下一次拖拽时立即隐藏旧工具栏。
