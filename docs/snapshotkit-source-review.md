...
HaxShot MVP 不需要完整的 `AppState` 单例，可以拆成 Flutter 的页面状态和 Rust 的平台服务。
...
这对 HaxShot 的直接意义：桥接接口不能只返回图片，还应返回：
...
HaxShot 使用 Flutter 画面模拟冻结背景，因此不需要完全复制 AppKit 的清除绘制方式，但必须保留两个原则：
...
这是 HaxShot 当前必须保留的原则：**原图和标注模型分离，最终保存和复制共享同一个渲染出口**。MVP2 的矩形、箭头和文字已经通过 `CapturePage._renderSelection()` 统一合成。
...
Pin 不属于 HaxShot 当前 MVP，但未来实现时要保留“管理器持有独立窗口对象”的生命周期设计。
...
二者都需要额外权限和大量桌面兼容工作，不进入 HaxShot MVP。
...
这些问题说明 HaxShot MVP 不应复制完整项目，而应只抽取坐标、图像输出和状态边界。
...
如果未来直接复制其代码，应保留 MIT 许可证和版权声明。当前 HaxShot 只参考架构和实现思路，不直接复制 Swift 代码。
