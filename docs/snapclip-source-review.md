...
HaxShot 原先通过 `org.freedesktop.portal.Screenshot` 请求整屏 PNG。这个接口适合普通应用获取授权后的截图，但在 GNOME Wayland 下会经过 Mutter 的截图路径，可能出现截图闪光和相机声音。
...
HaxShot 采用与 snapclip 相同的 ScreenCast 思路，但把实现放进 Rust：
...
一句话总结：**GNOME 原生体验的核心是先缓存 Mutter 合成器的一帧，再显示冻结遮罩；HaxShot 用 Mutter ScreenCast + PipeWire 在外部应用中复现这一顺序。**
