...
文字框形成后，四个角都是位于框外的缩放控制点。拖动任意角会保持文字框比例，并按缩放比例同步增大或减小字号；文字框顶部中央的抓手可以拖动整个文字框，抓手上方的关闭按钮可以删除当前文字标注。文字框边框、控制点、抓手和关闭按钮只属于编辑态，不会被写入最终 PNG。点击已有文字区域会再次进入编辑模式；点击其他区域会先提交当前文字，再创建新的文字输入点。
...
MVP2 暂时只支持矩形、箭头和文字，不支持对已提交的矩形/箭头进行选中移动、删除单个标注、马赛克和撤销。按下矩形或箭头工具后可以连续绘制多个标注；文字输入完成后可立即拖动四角缩放字号、使用顶部抓手移动或点击关闭按钮删除；点击“框选截图区域”可以重新选择截图范围并清空旧标注。

一句话总结：**第一次拖拽决定图片，后续操作绘制和编辑标注，保存和复制都使用同一份带标注的 PNG。**

版本发布流程见 [CI 与 GitHub Release](./ci-release.md)。
